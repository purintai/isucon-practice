...
SQL
    ...
    def friends
      @friends ||= redis.smembers("R_#{session[:user_id]}")
    end
...
SQL
    ...
    friends = redis.scard("R_#{current_user[:id]}")
...
SQL
  ...
  get '/initialize' do
    ...
    o, e, s = Open3.capture3("/bin/systemctl stop redis-server.service" + ';')
    o, e, s = Open3.capture3("/bin/cp /var/lib/redis/init.rdb /var/lib/redis/dump.rdb" + ';')
    o, e, s = Open3.capture3("/bin/systemctl start redis-server.service" + ';')
